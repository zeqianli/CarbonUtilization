...
CARBONS=['Arabinose','Glucuronic acid','Glycerol','Mannitol','Mannose','Deoxyribose','Melibiose','Butyrate','Propionate','Raffinose']
...
class BinaryGrowthClassifier(BaseEstimator):
    ...
    def score(self, X, y):
        ...
        score={'true_negative':cm[0,0],'false_positive':cm[0,1],'false_negative':cm[1,0],'true_positive':cm[1,1]}

        score['accuracy']=accuracy_score(y,prediction) # TODO: deal with zeros 
        
        if (score['true_positive']+score['false_positive'])>0:
            score['precision']=score['true_positive']/(score['true_positive']+score['false_positive'])
        else:
            score['precision']=np.nan

        if (score['true_positive']+score['false_negative'])>0:
            score['sensitivity']=score['true_positive']/(score['true_positive']+score['false_negative'])
        else:
            score['sensitivity']=np.nan

        if (score['true_negative']+score['false_positive'])>0:
            score['specificity']=score['true_negative']/(score['true_negative']+score['false_positive'])
        else:
        ...
        if (not np.isnan(score['precision'])) and (not np.isnan(score['sensitivity'])) and (score['precision']+score['sensitivity'])>0:
            score['f1_score']=2*score['precision']*score['sensitivity']/(score['precision']+score['sensitivity'])
        else:
...
class FBAClassifier(BinaryGrowthClassifier):
    ...
    def __init__(self, growth,c=None,growth_threshold=0.01):
        if isinstance(growth,str):
            growth=pd.read_csv(growth,index_col=0) # .drop(index=['EX_mobd_e']) # I don't know why there is an extra row at the end
...
class RandomGrowthNull(NullClassifier):
    ...
    def fit(self, X, y):
        y_shuffled=pd.Series(np.random.choice(y.values, size=len(y), replace=self.replace),index=y.index)
...
class LassoLogisticRegression(BinaryGrowthClassifier):
    def __init__(self, penalty='l1',C=1.0,solver='liblinear',**kwargs):
        self.model=LogisticRegression(penalty=penalty,C=C,solver=solver,**kwargs)
...
class GreedyFeatureSelection(BinaryGrowthClassifier):
    ...
    def fit(self,X,y):
        ...
        for n_feature in range(self.n_max_features):
                ...
                for i_split,(train_samples, test_samples) in enumerate(splits):
                    ...
                    if self.threads<=1:
                        ...
                        if (len(_data_split_batch) >= self.multithreading_batch) or (i_feature==len(iter_)-1 and i_split==len(splits)-1):
                            ...
                            result=self._p.starmap(_pickleable_run_model,_data_split_batch)
            ...
            results=results.groupby('new_feature')[self.pick_feature_metric].mean().sort_values(ascending=False)
        ...
        self._meta_training_data=pd.concat(self._meta_training_data,axis=0, ignore_index=True)
    ...
    for (c1, c1_dist, c1_root_dist), (c2, c2_dist, c2_root_dist) in itertools.combinations(zip(children, children_dist, _root_dists), 2):
        for c1_leaf, c1_leaf_dist in c1_root_dist.items():
            for c2_leaf, c2_leaf_dist in c2_root_dist.items():
                all_pairwise.append([c1_leaf, c2_leaf, c1_leaf_dist+c2_leaf_dist+c1_dist+c2_dist])
...
class DistMatOOCSplitter(OOCSplitter):

    def __init__(self, tree, growth_data, distance_matrix=None, ** kwargs):
        super().__init__(tree, growth_data, **kwargs)
    ...
    def compute_splits(self, samples):
        ...
        distance_matrix=self.distance_matrix.loc[samples,:].loc[:,samples]

        clades,clades_stat,clades_str=[],[],set()
        clade_size_range=(int(self.test_set_range[0]*len(samples)),int(self.test_set_range[1]*len(samples)))
        for s in samples:
            argind=np.argsort(distance_matrix.loc[s,:].values)
            neighbors=distance_matrix.columns.values[argind]
            for i in range(clade_size_range[0],clade_size_range[1]):
                clade=list(neighbors[:i])
                clade_str=','.join(np.sort(clade))
                if clade_str not in clades_str:
                    clades.append(clade)
                    clades_str.add(clade_str)
                    clades_stat.append({'clade':clade,'clade_str':clade_str,'clade_size':len(clade),"max_distance":distance_matrix.loc[clade,:].loc[:,clade].max().max()})
...
class TreeTraverseOOCSplitter(OOCSplitter):
    def __init__(self, tree, growth_data,
            ...
            super().__init__(tree, growth_data,test_set_range=test_set_range,min_zeros=min_zeros,min_ones=min_ones)
            self.max_clade=2
    ...
    def compute_splits(self, samples):
        ...
        for l in tree.get_leaves():
            if l.name not in samples:
                l.delete() # This may leave empty leaves. May include empty leave names in the next step. 
        ...
        for n_merge in range(1,self.max_clade+1):
            for clades_merge in tqdm(list(itertools.combinations(single_clades,n_merge))):
                ...
                if clade_str not in clades_str:
                    clades.append(clade)
                    clades_str.add(clade_str)
                    clades_stat.append({'clade':clade,'n_merge':n_merge,'clade_str':clade_str,'clade_size':len(clade)}) 
...
class LargeTreeTraverseOOCSplitter(DataSplitter):
    ...
    def split(self, samples, **kwargs):
        ...
        while True:
            if self.prefer_small_clade:
                clades=random.sample(self.single_clades[1:],np.random.randint(self.n_max_clade)+1)    
            else:
    ...
    def compute_single_clades(self, tree, samples):
        ...
        tree=tree.copy() # TODO: this can be expensive. Test if this matters. 
        tree_cleaned=False
...
def _pickleable_run_model(model,X_train,X_test,y_train,y_test,params,save_model):
    ...
    params.update(score)
    params.update({'train_sample_size':len(params['train_samples']),'test_sample_size':len(params['test_samples'])})
    if save_model: 
        try:
...
class PredictionPipeline:
    ...
    Parameters:
    -----------
    split: mode of data splits. Options:
        'random': random split. random_split_ratio is required. 
        TODO
        dict: user specified dictioinary
    ...
    def __init__(self,Model,model_params=None,threads=1,
        ...
        self.Model=Model
        if (self.Model==RFMetaLearning or self.Model==GreedyFeatureSelection) and threads>1:
        ...
        self.splits=None # a list of sample labels for data splits. [(c, train_samples, test_samples),...]
        self.X=None
        self.y=None
        self._data_split_batch=None # a batch of data splits that is feeded into _pickleable_run_model for multi-threading. This is done in batch for save memory. [(c, X_train, X_test, y_train, y_test, params),...]
        ...
        if isinstance(self.split_method,type) or self.split_method in ['random','ooc','ooc_distmat','leave_one_out']:
            ...
            for c in self.carbons:
                ...
                if isinstance(self.split_method,type):
                    ...
                    splitter=LargeTreeTraverseOOCSplitter(tree=self.tree,growth_data=y[c],**self.splitter_params)
                elif self.split_method=='ooc_distmat':
                    splitter=DistMatOOCSplitter(tree=self.tree,growth_data=y[c],**self.splitter_params)
                elif self.split_method=='leave_one_out':
                    splitter=LeaveOneOutSplitter(**self.splitter_params)
            ...
            self.splits=self._generate_splits_from_dict(X,y,self.split_method)
        else:
    ...
    def _generate_splits_from_dict(self,X,y,split_dict):
        ...
        if self.verbose:
            ...
            _iter=self.carbons
        for c in _iter:
            sample_mask=y[y[c].notna()].index.values # filter out samples with no growth data
    ...
    def run(self):
        ...
        for i_split,(c, train_samples, test_samples) in enumerate(tqdm(self.splits,desc='Training models...')):
            X_train,X_test=self.X.loc[train_samples,:],self.X.loc[test_samples,:]
            y_train,y_test=self.y.loc[train_samples,c],self.y.loc[test_samples,c]
            if not self.allow_empty_set:
                if X_train.shape[0]==0 or X_test.shape[0]==0:
                    continue
                if len(y_train)==0 or len(y_test)==0:
                    continue
            param=[self.Model(**(self.model_params[c])),X_train,X_test,y_train,y_test]
            param.append({'carbon_name':c,'train_samples':train_samples,'test_samples':test_samples})
...
def one_sample_test(arr1,arr2,sign='>'):
    if len(arr1)!=1 or len(arr2)==1:
        raise ValueError("First array length must be 1 and second array length must be greater than 1")
    ...
    return {'p':p,'significant':bool(p<0.05)}
...
def compare_models(df, model_pairs, 
    ...
    for name, group in gb:
        for _ in model_pairs:
            ...
            try:
                arrs=[group[group[model_key]==model][metric].values for model in models]
                out=f(*arrs)
...
def plot_tree_matrix(tree,matrix,cbar=True,
    ...
    fig,axes=plt.subplots(1,1+len(matrix),figsize=figsize,gridspec_kw={'width_ratios':width_ratio})

    Phylo.draw(tree_bio,axes=axes[0],do_show=False,**tree_params)# show_branch_support=False,show_scale=False,show_branch_length=False,
    axes[0].set_xticks([])
    axes[0].set_yticks([])
    axes[0].set_xlabel('')
    axes[0].set_ylabel('')
...
def cluster_matrix(matrix, axis=0, metric=None,**kwargs):
    ...
    if metric is None:
        def metric(x,y):
            x=(x>0).astype(int)
            y=(y>0).astype(int)
            return 1-(x==y).sum()/len(x) # -np.logical_and(np.isnan(x),np.isnan(y)).sum())
    Z=hierarchy.linkage(matrix, metric=metric,**kwargs)
    return matrix.iloc[hierarchy.leaves_list(Z),:]

def plot_prediction_pipeline_results(results, growth_data,tree,carbons=CARBONS,metric='accuracy',figsize=None,tree_width=10,matrix_width=10):
    ...
    for c,group in results.groupby('carbon_name'):
        ...
        split_matrix=pd.DataFrame(np.nan, index=growth_data.index, columns=[c]+list(range(group.shape[0])))
        split_matrix.iloc[:,0]=growth_data.loc[:,c]
        for i,(_, row) in enumerate(group.iterrows()):
            split_matrix.loc[row['test_samples'],i]=row[metric]
        split_matrix.iloc[:,1:]=cluster_matrix(split_matrix.iloc[:,1:],axis=1,optimal_ordering=True).values
        matrices.append(split_matrix)
        titles.append(c+'_'+metric)
        cbar.append(True)
        width_ratio.append(matrix_width)

    fig,axes=plot_tree_matrix(tree,matrices,titles=titles,cbar=cbar,figsize=(5+5*len(carbons),20),width_ratio=width_ratio)
    return fig
...
def plot_model_comparison(models, 
            ...
            if p_pair is None: # all model - null pairs
                for model_label in model_labels:
                    for null_label in null_labels:
                        pvalues.append({'carbon_name':c,'model1':model_label,'model2':null_label,'pvalue':np.nan})
            else:
                pvalues.append({'carbon_name':c,'model1':p_pair[0],'model2':p_pair[1],'pvalue':np.nan})

    model_gb=models_concat.groupby(['model_label','carbon_name'])
    for d in pvalues:
        m1,m2=model_gb.get_group((d['model1'],d['carbon_name'])),model_gb.get_group((d['model2'],d['carbon_name']))
        try:
    ...
    if axes is None:
        fig,axes=plt.subplots(len(carbons),2,figsize=figsize,gridspec_kw={'width_ratios':[1,0.5]})
    ...
    for (ax0,ax1),(c, models_group),(_, pvalues_group)  in zip(axes,models_concat.groupby('carbon_name'),pvalues.groupby('carbon_name')):
        ...
        if models_group.shape[0]>1:
            sns.violinplot(x='model_label',y=metric,data=models_group,ax=ax0,hue='is_null',order=[*model_labels,*null_labels])
        sns.stripplot(x='model_label',y=metric,data=models_group,ax=ax0,hue='is_null',order=[*model_labels,*null_labels])
        ax0.set_title(c)
        ax0.grid()
        ...
        sns.scatterplot(x='model_pair',y='pvalue',data=pvalues_group,ax=ax1,hue='significant')
        ax1.grid()
...
# def _pickleable_rf_meta_learning_run(pipe, features, X,y):
#     pipe.generate_splits(X.loc[:,features],y)
#     results=pipe.run()
#     results['meta_model_features']=';'.join(features)
#     return results


# class RFMetaLearning(BinaryGrowthClassifier):
#     def __init__(self, 
#             tree, 
#             n_meta_clade=5,n_meta_feature=10,n_meta_models=100,
#             meta_rf_params=None,
#             #meta_rf_n_tree=10,
#             ooc_method='ooc', ooc_split_params=None,
#             #meta_split_range=(0.2,0.3),
#             pick_feature_metric='accuracy',n_top_models=100,n_best_features=10,
#             threads=1,save_meta_models=False):
#         # clade spliting parameters 
#         self.tree=tree
#         self.n_meta_clade=n_meta_clade
#         self.meta_rf_params=meta_rf_params

#         # meta learning parameters
#         self.n_meta_feature=n_meta_feature
#         self.n_meta_models=n_meta_models
#         #self.meta_rf_n_tree=meta_rf_n_tree
#         self.ooc_method=ooc_method
#         self.ooc_split_params=ooc_split_params
...
#         starmap_params=[]
#         for _ in range(self.n_meta_models):
#             pipe=PredictionPipeline(RF,
#                 model_params=self.meta_rf_params,
#                 split_method=self.ooc_method,splitter_params=self.ooc_split_params,
#                 threads=1,
#                 n_splits=self.n_meta_clade, 
#                 tree=self.tree, 
#                 carbons=['_dummy'],verbose=False,save_models=self.save_meta_models)
#             features=np.random.choice(X.columns.values,self.n_meta_feature,replace=False)
#             starmap_params.append((pipe,features,X,y.to_frame(name='_dummy')))
        
#         print("Running meta models...")
        
#         if self.threads<=1:
#             results=list(itertools.starmap(_pickleable_rf_meta_learning_run,starmap_params))
#         else:
#             with Pool(self.threads) as p:
#                 results=p.starmap(_pickleable_rf_meta_learning_run,starmap_params)
#         results=pd.concat(results,axis=0)
        
#         self._meta_training_data=results    
        
#         print("Selcting best features...")
#         self.best_features=self.pick_top_features(results,n_top=self.n_top_models,n_feature=self.n_best_features)

#         # Train a final model using the best features 
#         model=RFSelectedFeatures(self.best_features, max_features=None)
...
#     def pick_top_features(self,meta_models,n_top=100,n_feature=10):
#         top_features=meta_models.groupby(['meta_model_features']).apply(lambda x: x['accuracy'].mean()).sort_values(ascending=False).index.values[:n_top]

#         meta_top_results=meta_models[meta_models['meta_model_features'].isin(top_features)]
#         df_feature_importance=[]

#         for _, row in meta_top_results.iterrows():
#             for ko, fi in zip(row['meta_model_features'].split(';'),row['feature_importances']):
#                 df_feature_importance.append({'ko':ko,'fi':fi,'appearance':1})
#         df_feature_importance=pd.DataFrame(df_feature_importance).groupby('ko').sum().sort_values(by='fi',ascending=False)
#         return df_feature_importance.iloc[:n_feature].index.values
    
#     def get_params(self,deep=True):
#         return {'best_features':self.best_features,'final_model':self.model}

# ============================================================
# Calculate p-values 
# ============================================================

# def cal_p_value(data, data_shuffled,metric='accuracy',sign='>=',groupby=['carbon_name']):
#     data=data.copy()
#     if isinstance(groupby,list) and len(groupby)==1:
#         groupby=groupby[0]
#     gb_shuffled=data_shuffled.groupby(groupby)

#     def get_p_value(row,metric='cm_accuracy',sign='>='):
#         # Get p-value for each carbon source
#         if isinstance(groupby,list):
#             gb=gb_shuffled.get_group(tuple(row[groupby].values))
#         elif isinstance(groupby,str):
#             gb=gb_shuffled.get_group(row[groupby])
#         else:
#             raise ValueError
#         if sign=='>=':
#             p_value=(gb[metric]>=row[metric]).mean()
#         elif sign=='<=':
#             p_value=(gb[metric]<=row[metric]).mean()
#         else: 
...
#     data['p_value_'+metric]=data.apply(lambda row: get_p_value(row, metric=metric,sign=sign),axis=1)
#     return data


# Deprecated. Use tree.prune(samples, preserve_branch_length=True)
# def trim_tree(tree, samples=None):
#     if samples is None:
#         return tree
...
# class RFSelectedFeatures(RF):
#     def __init__(self, selected_features, **kwargs):
#         self.selected_features=selected_features
#         super().__init__(**kwargs)
    
#     def fit(self, X, y):
#         return super().fit(X.loc[:,self.selected_features], y)
    
#     def predict(self, X):
#         return super().predict(X.loc[:,self.selected_features])
    
#     def get_params(self, deep=True):
#         out={'selected_features':self.selected_features}
#         out.update(super().get_params(deep))
#         return out        

# def calculate_distance_matrix(tree):
#     leaves=[l.name for l in tree.get_leaves()]
#     distance_matrix=pd.DataFrame(index=leaves,columns=leaves)
#     for i in tqdm(leaves,desc='Calculating distance matrix...'):
#         for j in leaves:
#             if i!=j:
#                 distance_matrix.at[i,j]=tree.get_distance(i,j)
