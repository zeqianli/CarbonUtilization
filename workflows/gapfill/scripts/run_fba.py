...
def load_bigg():
    ...
    # only one duplicate bigg_id "(GlcNAc)7 (Man)3 (Asn)1"
    bigg = bigg.drop_duplicates(subset=['bigg_id'])
...
def find_c_condidates(model):
    ...
    # model.medium should be same for all models. This is initialized by the -i tag in carveme?
    c_candidates = np.setdiff1d(bigg_e, list(model.medium.keys()))
    return c_candidates
...
def set_force_uptake(model, metabolite_id):
    ...
    metabolite_cobra = cobra.Metabolite(
        metabolite_id + '_e', name=metabolite_name, compartment='C_e')
    try:
    ...
    metabolite_p_cobra = cobra.Metabolite(
        metabolite_id + '_p', name=metabolite_name, compartment='C_p')
    try:
    ...
    metabolite_c_cobra = cobra.Metabolite(
        metabolite_id + '_c', name=metabolite_name, compartment='C_c')
    try:
    ...
    try:
        ...
        reaction_diff.add_metabolites(
            {metabolite_cobra: -1, metabolite_p_cobra: 1})
    ...
    reaction_uptake.add_metabolites({metabolite_p_cobra: -1, model.metabolites.get_by_id('atp_c'): -1,
                                    metabolite_c_cobra: 1,  model.metabolites.get_by_id('adp_c'): 1, model.metabolites.get_by_id('pi_c'): 1})
...
def add_external_metabolite(model, metabolite_id):
    metabolite_name = bigg.at[metabolite_id+'_e', 'name']
    metabolite_cobra = cobra.Metabolite(
        metabolite_id, name=metabolite_name, compartment='C_e')
    try:
        x = model.metabolites.get_by_id(metabolite_id+'_e')
    except KeyError:
        model.add_metabolites(metabolite_cobra)
...
def initialize_medium(model, metabolite_ids, default_flux=10):
    ...
    model.medium = {'EX_'+metabolite_id +
                    '_e': default_flux for metabolite_id in metabolite_ids}
    ...
    for c in c_candidates:
        try:
            with model:  # Ahmed's code is wrong here. He put with outside the loop. That forces uptakes of all bigg metabolites. The results might be same, since the lower bound is changed to zero at the end, but it might confuses the optimization.
                ...
                # Turn on the candidate exchange (uptake) # TODO: is this the same thing as setting medium?
                model.reactions.get_by_id(c).lower_bound = -10
                if force_uptake:
                    set_force_uptake(model, c[3:-2])
                growth = model.slim_optimize()
                growths[c] = growth
                # Turn off the candidate exchange (uptake). Although this shouldn't be necessary.
                model.reactions.get_by_id(c).lower_bound = 0
        except KeyError:
            print('Error with', c)
            growths[c] = np.nan
...
def main(force_uptake, output, initial_medium, model):
    base_medium = pd.read_csv(initial_medium, sep='\t', header=None, names=[
                              'media', 'description', 'bigg_id', 'metabolite_description'])['bigg_id'].values
